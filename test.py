...
def args_parse():
    ...
    ap.add_argument("-name","--model_name", default="MergeNet.h5",
...
def test(args):
    ...
    print(len(X_train))
    for i in range(1):
        y_pred = model.predict([X_train[i].reshape((-1, 320, 480, 3)),X_mask[i].reshape((-1,320,480,1))])
        ...
        X_bicubic = cv.normalize(X_bicubic, None, 0, 255, cv.NORM_MINMAX, cv.CV_8U)
        ...
        ref = cv.normalize(ref, None, 0, 255, cv.NORM_MINMAX, cv.CV_8U)
